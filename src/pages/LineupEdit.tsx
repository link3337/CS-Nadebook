...
const toNormalized = (px: number, py: number) => [px / viewBoxWidth, py / viewBoxHeight] as [number, number];
...
const LineupEdit: React.FC = () => {
  ...
  const [startCoords, setStartCoords] = useState<[number, number] | undefined>(lineup?.startCoords as any);
  const [targetCoords, setTargetCoords] = useState<[number, number] | undefined>(lineup?.targetCoords as any);
    ...
    <div style={{ padding: 16, display: "flex", gap: 16 }}>
        ...
        <div style={{ display: "grid", gap: 8, maxWidth: 640 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button onClick={() => setMode(mode === "start" ? "none" : "start")} style={{ background: mode === "start" ? "#ddd" : undefined }}>
                {mode === "start" ? "Setting start (click map)" : "Set Start"}
              </button>
              <button onClick={() => setMode(mode === "target" ? "none" : "target")} style={{ background: mode === "target" ? "#ddd" : undefined }}>
                {mode === "target" ? "Setting target (click map)" : "Set Target"}
              </button>
              <button onClick={() => { setStartCoords(undefined); setTargetCoords(undefined); }}>Clear</button>
            </div>
          </div>

          <div style={{ border: "1px solid #ccc", borderRadius: 8, overflow: "hidden" }}>
            <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} width="100%" height={400} onClick={onSvgClick} style={{ display: "block" }}>
              {mapImage && <image href={mapImage} x={0} y={0} width={viewBoxWidth} height={viewBoxHeight} preserveAspectRatio="xMidYMid slice" />}
              {/* markers */}
              {startCoords && (() => {
                const p = toViewBox(startCoords);
                return <circle cx={p![0]} cy={p![1]} r={10} fill="#4dabf7" stroke="#000" strokeWidth={1} />;
              })()}
              {targetCoords && (() => {
                const p = toViewBox(targetCoords);
                return <circle cx={p![0]} cy={p![1]} r={8} fill="#ffd43b" stroke="#000" strokeWidth={1} />;
              })()}
              {startCoords && targetCoords && (() => {
                const s = toViewBox(startCoords)!;
                const t = toViewBox(targetCoords)!;
                return <line x1={s[0]} y1={s[1]} x2={t[0]} y2={t[1]} stroke="#ff6b6b" strokeWidth={3} strokeOpacity={0.9} />;
              })()}
            </svg>
          </div>
      ...
      <aside style={{ width: 360 }}>
        ...
        <div style={{ border: "1px solid #ddd", padding: 8, borderRadius: 6 }}>
          <div><strong>Start:</strong> {startCoords ? `${startCoords[0].toFixed(3)}, ${startCoords[1].toFixed(3)}` : "—"}</div>
          <div><strong>Target:</strong> {targetCoords ? `${targetCoords[0].toFixed(3)}, ${targetCoords[1].toFixed(3)}` : "—"}</div>
        </div>
      </aside>
    </div>
  );
